...
Palmetto 2 supercomputing cluster. If you have an account on Palmetto 2,
you can log in as described in [our manual](https://www.palmetto.clemson.edu/palmetto/basic/login/).
...
- To quit the R console, type `quit()`. Let's create a simple script
and run it. We can use the text editor called `nano`:

~~~bash
nano randmatrix.r
~~~

- This will open a text editor. Let's paste these lines which create
a random 4x4 matrix (to paste, Mac users can use Cmd+V, and PC users
can use Shift+Ins):
...
- Let's save it by pressing `Ctrl+O`, then `Enter`, then `Ctrl+X`. We
can run this script with the `RScript` command:
