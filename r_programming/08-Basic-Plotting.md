...
built-in functions for plotting. Let's look at some of them and
apply them to the `mtcars` data set.
...
`main` (main figure title), and `xlab` and `ylab` (X- and Y-axis labels). We
...
- Next, make a scatter plot. We'll use the `plot` function for this purpose.
...
- The most common place for a plot to be "sent" is the screen device
  ...
  - You can save your graphic to PDF, PNG, or JPEG
