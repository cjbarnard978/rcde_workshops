...
- Let's save the function as max.eig.R in our working directory. Then, we'll need
...
- Now, since we are running the code in parallel, it should take less time to run. Let's get the accurate timing with the `system.time` command:
