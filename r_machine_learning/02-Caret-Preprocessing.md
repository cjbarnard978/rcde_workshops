...
- The caret package (short for **C**lassification **A**nd **RE**gression **T**raining) is a set of functions that attempt to streamline the process for creating predictive models. The package contains tools for, among other things:
  - pre-processing (cleaning up the data)
  - data splitting (separating the data into training and test sets)
  - feature selection (selecting a subset of variables for further analysis)
...
a uniform interface of the functions themselves, as well as a way to standardize common
...
There are several steps that we will use `caret` for. For preprocessing raw data, we will use `caret` for these tasks:
- Pre-processing with missing values
...
in our data. Interestingly, you can see an example of
