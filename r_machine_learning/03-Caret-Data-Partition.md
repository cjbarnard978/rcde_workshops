...
outcomes.
  ...
  perform well when it is applied to data that it has never seen before.
...
![5-fold cv](../fig/r_ml/5fold_cv.png)
  ...
  - split-half: the data are evenly split into training and test sets. First, you use one half for training and the other for testing; second, you use the second half for training and the first half for testing. The amount of training data is relatively small, but this way is sometimes used if the stability of the model is just as important as the classification accuracy.
...
More information on model tuning using caret can be found at [Caret: Model Training and Tuning](https://topepo.github.io/caret/model-training-and-tuning.html)
